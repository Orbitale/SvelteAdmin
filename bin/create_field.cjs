...
async function configureNewFieldName() {
	...
	fieldName.fullSnakeCase = (fieldName.fullPascalCase.charAt(0).toLowerCase() + fieldName.fullPascalCase.slice(1)).replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`);
	fieldName.fullPascalCase = fieldName.fullPascalCase.charAt(0).toUpperCase() + fieldName.fullPascalCase.slice(1);
...
async function updateThemeConfigType() {
	...
	const typesConfig = (await fs.readFile(typesFile, 'utf8'))
		.replace(/(viewFields:[^}]+)(\[key: string]: ComponentType;)$/gm, `$1${fieldName.baseSnakeCase}: ComponentType;\n\t\t$2`)
...
async function reformatFiles() {
	...
	const command = spawn(nodePath, [yarnPath, 'run', 'prettier', '--plugin=prettier-plugin-svelte', '--write', ...filesToFormat], {
		cwd: projectDir
	});
