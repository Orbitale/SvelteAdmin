...
import { getSubmittedFormData } from '$lib/Crud/Form.js';
describe('Submitted form data', () => {
	it('does not work if no target is specified', () => {
	...
	it('can handle input containing the same key more than once', () => {
		const submitted = getSubmittedFormData(
			mockSubmitEvent([
				['title', 'First title'],
				['title', 'Second title']
