<script lang="ts">
	...
	import { ISO_COUNTRIES } from '$lib/Fields/internal/intlData';
	...
	try {
		...
		console.warn('Intl.DisplayNames unavailable for regions:', e);
	}

	$: codes = (only && only.length > 0 ? only : ISO_COUNTRIES).slice().sort();
