...
export class InMemoryStorage<T extends Entity> implements InternalStorage<T> {
    ...
    public get(id: string|number): T {
        ...
        if (found.length !== 1) {
            throw new Error(`Error: Found multiple objects of type "${this.objectName}" with id "${id}".`);
        }
    ...
    public add(object: T): void {
        ...
        if (item) {
            throw new Error(`Attempted to create new object of type "${this.objectName}", but its ID was already found. Did you mean to use "update" instead?`);
        }
